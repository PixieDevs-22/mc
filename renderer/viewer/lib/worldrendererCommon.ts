...
export const defaultWorldRendererConfig = {
  ...
  // Performance settings — optimized for low-end devices
  mesherWorkers: 2, // reduced from 4 to lower CPU/memory overhead
  addChunksBatchWaitTime: 100, // reduced from 200ms for faster chunk loading
  _experimentalSmoothChunkLoading: false, // disabled: saves main-thread time per frame
...
export abstract class WorldRendererCommon<WorkerSend = any, WorkerReceive = any> {
  ...
  ONMESSAGE_TIME_LIMIT = 8 // ms — tightened from 30ms to fit within 16ms frame budget at 60fps
  ...
  dispatchMessages () {
    ...
    // Use queueMicrotask (lower latency than setTimeout) to batch all dirty
    // sections added in the same synchronous call stack, then dispatch once.
    queueMicrotask(() => {
      // group messages and send as one batch per worker — fewer postMessage calls
