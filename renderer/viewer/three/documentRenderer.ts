...
export class DocumentRenderer {
  ...
  constructor (initOptions: GraphicsInitOptions, public externalCanvas?: OffscreenCanvas) {
    ...
    try {
      this.renderer = new THREE.WebGLRenderer({
        canvas: this.canvas,
        preserveDrawingBuffer: false, // disabled: saves GPU memory bandwidth (screenshots use renderer.domElement directly)
        logarithmicDepthBuffer: false, // disabled: log depth is expensive, standard depth sufficient for Minecraft
        antialias: false, // disabled: MSAA is expensive; Minecraft blocks don't need it
        powerPreference: this.config.powerPreference,
        precision: 'mediump', // use medium precision for better mobile GPU performance
      })
    } catch (err) {
  ...
  // Low-end optimization: cap pixel ratio to reduce GPU fill rate
  private applyLowEndPixelRatio () {
    const cap = 1.5 // never render more than 1.5x native pixels
    const pr = Math.min(window.devicePixelRatio || 1, cap)
    this.renderer.setPixelRatio(pr)
  }

  private startRenderLoop () {
    // Apply pixel ratio cap immediately for low-end devices
    if (!this.externalCanvas) {
      this.applyLowEndPixelRatio()
    }

    // Target 60fps: interval in ms
    const TARGET_FPS = 60
    const TARGET_FRAME_MS = 1000 / TARGET_FPS

    // Accumulated time for drift-free frame pacing
    let accumulator = 0
    ...
    const animate = (now: number) => {
      ...
      if (this.config.timeoutRendering) {
        this.timeoutId = setTimeout(() => animate(performance.now()), this.config.fpsLimit ? 1000 / this.config.fpsLimit : 0) as unknown as number
      } else {
      ...
      // Clamp delta to avoid spiral of death after tab switch/freeze
      const clampedDelta = Math.min(delta, 100)

      const effectiveFpsInterval = this.config.fpsLimit ? 1000 / this.config.fpsLimit : TARGET_FRAME_MS
      accumulator += clampedDelta

      if (accumulator < effectiveFpsInterval - 0.5) {
        // Not yet time for next frame — skip to save CPU/GPU
        return
      }

      // Consume one frame worth of time (prevents speeding up after lag)
      accumulator = Math.min(accumulator - effectiveFpsInterval, effectiveFpsInterval)
