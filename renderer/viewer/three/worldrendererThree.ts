...
export class WorldRendererThree extends WorldRendererCommon {
  ...
  material = new THREE.MeshBasicMaterial({ vertexColors: true, transparent: true, alphaTest: 0.5 }) // raised alphaTest from 0.1 to 0.5 — skips more semi-transparent pixels, reduces overdraw
  ...
  render (sizeChanged = false) {
    ...
    // Adaptive entity update: on low-end devices or when frames are dropping,
    // only update entities every other frame to halve entity CPU cost.
    const skipEntityThisFrame = this._slowFrameCount > 3 && (this._entityUpdateFrame++ % 2 !== 0)
    if (!this.reactiveDebugParams.disableEntities && !skipEntityThisFrame) {
    ...
    // Only run fountains/particles if we have headroom (frame time under 14ms)
    const midTime = performance.now() - start
    const hasHeadroom = midTime < 14
    ...
    // Track slow frames (> 16ms = under 60fps)
    if (totalTime > 16) {
      this._slowFrameCount = Math.min(this._slowFrameCount + 1, 10)
    } else {
      this._slowFrameCount = Math.max(this._slowFrameCount - 1, 0)
    }
